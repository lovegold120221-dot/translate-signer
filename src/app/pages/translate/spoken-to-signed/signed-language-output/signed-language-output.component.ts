...
import {getSignedLanguageProfile} from '../../../../modules/translate/signed-language-profiles';
...
export class SignedLanguageOutputComponent extends BaseComponent implements OnInit {
  ...
  signedLanguageCode: string;
  signedLanguageStatusMessage: string | null = null;
  ...
  ngOnInit(): void {
    ...
    this.signedLanguage$
      .pipe(
        tap(language => {
          this.signedLanguageCode = language;
          this.signedLanguageStatusMessage = getSignedLanguageProfile(language).note ?? null;
        }),
        takeUntil(this.ngUnsubscribe)
      )
      .subscribe();
