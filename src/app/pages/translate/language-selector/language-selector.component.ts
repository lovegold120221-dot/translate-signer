...
export class LanguageSelectorComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  langName(lang: string): string {
    ...
    const key = `${this.translationKey}.${lang}`;
    ...
    if (this.translationKey === 'signedLanguagesShort') {
      const signedLanguage = IANASignedLanguages.find(({signed}) => signed === lang);
      if (signedLanguage?.abbreviation) {
        return signedLanguage.abbreviation;
      }
    }
