...
export class TranslationService {
  ...
  normalizeSpokenLanguageText(language: string, text: string): Observable<string> {
    ...
    const url = `${getTranslationApiConfig().textNormalizationUrl}?${params.toString()}`;

    return this.http.get<{text: string}>(url).pipe(map(response => response.text));
  }

  describeSignWriting(fsw: string): Observable<string> {
    const url = getTranslationApiConfig().signWritingDescriptionUrl;
  ...
  translateSpokenToSigned(text: string, spokenLanguage: string, signedLanguage: string): string {
    const api = getSignedLanguageTranslationApi(signedLanguage).spokenTextToSignedPoseUrl;
    const backendLanguage = getSignedLanguageBackendCode(signedLanguage);
    return `${api}?text=${encodeURIComponent(text)}&spoken=${spokenLanguage}&signed=${backendLanguage}`;
  }
}
