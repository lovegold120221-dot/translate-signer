...
import {getSignedLanguageBackendCode, getSignedLanguageTranslationApi} from '../../core/config/translation-api';
...
export class SignWritingTranslationService {
  ...
  translateOnline(
    ...
    const backendSignedLanguage = getSignedLanguageBackendCode(to);
    const url = getSignedLanguageTranslationApi(to).spokenTextToSignWritingUrl;
    const body = {
      data: {
        texts: sentences.map(s => s.trim()),
        spoken_language: from,
        signed_language: backendSignedLanguage,
      },
    };
  ...
  translateSpokenToSignWriting(
    ...
    const backendSignedLanguage = getSignedLanguageBackendCode(signedLanguage);
    const offlineSpecific = () => {
      const newText = `${this.preProcessSpokenText(text)}`;
      return from(this.translateOffline(direction, newText, spokenLanguage, backendSignedLanguage));
    };

    const offlineGeneric = () => {
      const newText = `$${spokenLanguage} $${backendSignedLanguage} ${this.preProcessSpokenText(text)}`;
